...
class BruteForceSpec extends FlatSpec with Matchers {
  ...
  "The algorithm" should "terminate with the desired results" in {
    ...
    val groups = Set[Group](
      Group(id = 0, minSize = 3, maxSize = 3, name = "Group 0", skills = Set("1", "2", "3")),
      Group(id = 1, minSize = 3, maxSize = 3, name = "Group 1", skills = Set("1", "2", "3")),
      Group(id = 2, minSize = 3, maxSize = 4, name = "Group 2", skills = Set("1", "2", "3"))
    ).map(s => s.id -> s).toMap
...
//    assignment.studentMap should have size students.size
    ...
    val bruteForce = new BruteForce(course)
    val outcome = bruteForce.startSolving()

    outcome._1 should not be 0
    outcome._2 should have size groups.size
//    outcome._2 should be equal to assignment.groupMap
  }
}
